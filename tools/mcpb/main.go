// Command mcpb packs the capetown-opendata-mcp binary into an Anthropic MCP
// Bundle (.mcpb) — a zip archive containing a manifest.json at the root and the
// server binary under server/. It is a small, dependency-free (stdlib-only)
// alternative to the Node @anthropic-ai/mcpb CLI, so a bundle can be built on
// any OS with just the Go toolchain.
//
// Usage:
//
...
const (
	projectName = "capetown-opendata-mcp"
	displayName = "Cape Town Open Data"
	authorName  = "Richard Wooding"
	description = "MCP server for the City of Cape Town Open Data Portal"
)
	...
	m := Manifest{
		...
		UserConfig: map[string]UserConfigField{
			"arcgis_token": {
				Type:        "string",
				Title:       "ArcGIS token",
				Description: "Optional ArcGIS token for authenticated services",
				Sensitive:   true,
				Required:    false,
			},
			"timeout": {
				...
				Description: "Per-request upstream timeout (Go duration, e.g. 30s)",
				Default:     "30s",
			},
			"cache_ttl": {
				Type:        "string",
